...
mapLoop:
    lw t1, 0(s0)        # add t1, s0, x0 # load the address of the array of current node into t1 # Error: this assign the address of s0 instead of array
    lw t2, 4(s0)        # load the size of the node's array into t2
    slli t2, t2, 2      # Error: should convert into bits
    ...
    addi t0, t0, 4      # addi t0, t0, 1      # increment the count # Error: should increment 4 bits which is 1 byte
    bne t0, t2, mapLoop # repeat if we haven't reached the array size yet

    lw a0, 8(s0)        #la a0, 8(s0)        # load the address of the next node into a0 # Error: la only load address from label
    add a1, s1, x0      #lw a1, 0(s1) # put the address of the function back into a1 to prepare for the recursion # Error: s1 is the address of the function
